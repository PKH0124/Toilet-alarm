...
int32_t Abs_Lora_SetAT(void)
{
	...
	if(ret == 0)
	{
		ret = Bsp_Lora_Send_Cmd("AT+WMODE=MS\r\n","OK",300);
	}
	if(ret == 0)
	{
		ret = Bsp_Lora_Send_Cmd("AT+TMODE=1\r\n", "OK", 300);
	}
	if(ret == 0)
	{
		ret = Bsp_Lora_Send_Cmd("AT+PMODE=WU\r\n", "OK", 300);
	}
	if(ret == 0)
	{
		ret = Bsp_Lora_Send_Cmd("AT+WTM=2500\r\n", "OK", 300);
	}
	if(ret == 0)
	{
	...
	if(ret == 0)
	{
		ret = Bsp_Lora_Send_Cmd("AT+ADDR=0\r\n", "OK", 300);
	}
	if(ret == 0)
	{
